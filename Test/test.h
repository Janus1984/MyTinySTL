...
public:
    ...
    // 构造函数，接受一个字符串代表案例名称
    explicit TestCase(const char* case_name) : testcase_name(case_name) {}
...
void UnitTest::Run() {
    ...
    std::cout << GREEN << " " << nPassed << " / " << nFailed + nPassed << " TestCases passed. ( "
              << nPassed / (nFailed + nPassed) * 100 << "% )\n";
}
...
// 输出测试数量级
void test_len(size_t len1, size_t len2, size_t len3, int wide) {
    std::string str1;
    std::string str2;
    std::string str3;
    std::stringstream ss;
...
#define MAP_EMPLACE_DO_TEST(mode, con, count)                                               \
    do {                                                                                    \
        ...
        char buf[10];                                                                       \
        start = clock();                                                                    \
        for (size_t i = 0; i < (count); ++i) c.emplace(mode::make_pair(rand(), rand()));    \
        end = clock();                                                                      \
        int n = static_cast<int>(static_cast<double>(end - start) / CLOCKS_PER_SEC * 1000); \
        std::snprintf(buf, sizeof(buf), "%d", n);                                           \
        std::string t = buf;                                                                \
