...
namespace mystl::test::unordered_map_test {
...
void unordered_map_test() {
    ...
    std::cout << " um1.equal_range(3) : from <" << first.first << ", " << first.second << "> to <" << second.first
...
void unordered_multimap_test() {
    ...
    std::cout << "[-------------------------- API test ---------------------------]" << std::endl;
    mystl::vector<PAIR> v;
    for (int i = 0; i < 5; ++i) v.push_back(PAIR(5 - i, 5 - i));
